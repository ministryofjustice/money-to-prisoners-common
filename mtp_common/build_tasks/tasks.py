...
def local_docker(context: Context):
    ...
    context.shell('docker-compose', 'build', '--force-rm')
    context.shell('docker-compose', 'up', '' if context.use_colour else '--no-color',
                  environment={'HOST_MACHINE_IP': host_machine_ip})
