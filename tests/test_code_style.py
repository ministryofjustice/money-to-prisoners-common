...
class CommonCodeStyleTestCase(unittest.TestCase):
    ...
    node_bin_path = pathlib.Path('node_modules') / '.bin'
    ...
    def get_tool_bin(cls, tool):
        return cls.node_bin_path / tool

    @classmethod
    def install_tools(cls):
        if all(cls.get_tool_bin(tool).exists() for tool in cls.tools):
            return
        sys.stdout.flush()
    ...
    def get_config_file(self, file_name):
        return self.app.templates_path / 'mtp_common' / 'build_tasks' / file_name
    ...
    def test_stylesheets_code_style(self):
        self.run_node_tool(
            ...
            '--format', 'stylish',
            '--syntax', 'scss',
            self.app.scss_source_path / '**' / '*.scss'
        )
